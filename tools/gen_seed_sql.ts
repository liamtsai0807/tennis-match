...
import { CLUBS, COURTS, PLAYERS, OPEN_MATCHES } from '../src/lib/mockData.ts'
...
const sql =
  ...
  insert('open_matches', OPEN_MATCHES as unknown as Record<string, unknown>[])

writeFileSync(new URL('../supabase/seed.sql', import.meta.url), sql)
console.log('已產生 supabase/seed.sql（' +
  (CLUBS.length + COURTS.length + PLAYERS.length + OPEN_MATCHES.length) + ' 筆）')
