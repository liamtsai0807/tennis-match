...
- `level.test.ts`（11 個）程度推算：結果範圍、回合數是否確實主導、
  頻率權重是否高於年資、刻度對齊
- `flex.test.ts`（13 個）推播訊息的結構
- `richmenu.test.ts`（5 個）圖文選單：深連結指向的路由在 App 裡真的存在、
  底圖格數與點擊範圍一致、點擊範圍蓋滿整張圖沒有縫

共同點是**寫錯都不會當機，只會默默給錯的結果**——推薦錯的人、送出結構不對的
訊息、或是按下去開了一個空白頁。這種 bug 開發時不會有人發現，所以要有測試守著。
